...
const DashboardCard = ({ icon, title, description, path, color, actions = [] }) => {
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:shadow transition-all duration-200 overflow-hidden group">
      <div className="p-5">
        <div className="flex items-center justify-between mb-3">
          <div className={`inline-flex items-center justify-center rounded-md p-2 ${color}`}>
            {icon}
          </div>
        ...
        <h3 className="text-base font-medium text-gray-900 dark:text-gray-100 mb-1.5">{title}</h3>
        <p className="text-gray-600 dark:text-gray-400 text-xs mb-3">{description}</p>
          ...
          <div className="flex flex-wrap gap-2 mt-auto pt-2 border-t border-gray-100 dark:border-gray-700">
            {actions.map((action, index) => (
              <Link 
                key={index}
                to={action.path} 
                className="inline-flex items-center text-xs font-medium text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200"
              >
...
const QuickAction = ({ icon, label, to, color }) => {
  return (
    <Link
      to={to}
      className={`${color} flex flex-col items-center justify-center rounded-md p-3 hover:opacity-90 transition-all shadow-sm no-underline`}
    >
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          {isLoading ? (
            ...
            <h4 className="text-lg font-medium text-gray-900 dark:text-gray-100">{value}</h4>
          )}
        </div>
        <div className="p-2 rounded-md bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
          {icon}
        </div>
      </div>
    </div>
...
const ActivityItem = ({ activity, onRefresh }) => {
  ...
  return (
    <div className="flex items-start space-x-4 p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-750 transition-all duration-200">
      {/* Icon */}
      ...
      <div className="flex gap-2">
        {activity.activity_type === 'import_questions' && (
          <button
            ...
            className="inline-flex items-center px-2 py-1 border border-transparent rounded text-xs font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 dark:focus:ring-offset-gray-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        {activity.activity_type === 'import_questions' && (
          <button
            ...
            className="inline-flex items-center px-2 py-1 border border-gray-300 dark:border-gray-600 rounded text-xs font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-900 transition-colors"
...
const EmptyState = ({ message, icon }) => {
  return (
    <div className="flex flex-col items-center justify-center p-6 text-center">
      <div className="text-gray-400 dark:text-gray-500 mb-2">{icon}</div>
...
const Overview = () => {
  ...
  const safeApiCall = useCallback(async (endpoint, defaultValue = { count: 0 }) => {
    try {
      ...
      // Any error will silently return the default value
      return defaultValue;
    }
  }, [get]);

  // Combined fetch for all dashboard data to reduce useEffect dependencies
  useEffect(() => {
    // Only run this effect once
    if (hasRunOnceRef.current || apiErrorOccurred) {
      return;
    }
    ...
    const fetchAllDashboardData = async () => {
      try {
        ...
        // Create an array of promises that won't reject
        const apiCalls = [
          safeApiCall('/api/questions/count'),
          safeApiCall('/api/curriculum/count'),
          user?.role === 'admin' ? safeApiCall('/api/users/count') : Promise.resolve({ count: 0 }),
          safeApiCall('/api/activity/recent', { activities: [] })
        ];
        
        // Execute all API calls in parallel
        const [questions, curriculum, users, activity] = await Promise.all(apiCalls);
        
        // Update stats with fetched data
          setStats({
          questions: { value: questions.count || 0, isLoading: false },
          curriculum: { value: curriculum.count || 0, isLoading: false },
          users: { value: users.count || 0, isLoading: false },
          uploads: { value: user?.uploadCount || 0, isLoading: false }
        });
    ...
    // Execute the combined fetch function
    fetchAllDashboardData();
  }, [user, safeApiCall, apiErrorOccurred]);

  // Format dates for activity items
  ...
  const getActivityIcon = (type) => {
    switch (type) {
      case 'upload':
        return <FaCloudUploadAlt className="w-3.5 h-3.5 text-white" />;
      case 'edit':
        return <FaEdit className="w-3.5 h-3.5 text-white" />;
      case 'user':
        return <FaUsers className="w-3.5 h-3.5 text-white" />;
      case 'export':
        return <FaQuestionCircle className="w-3.5 h-3.5 text-white" />;
      default:
        return <FaCog className="w-3.5 h-3.5 text-white" />;
    }
  };
  ...
  // Generate placeholder activity when no real data is available
    ...
    return [
      {
        ...
        description: 'Your activity will appear here as you use the application',
  ...
  // Activity data to display (real or placeholder)
  const activityToDisplay = recentActivity.length > 0 ? recentActivity : getPlaceholderActivity();
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900">
      ...
      <div className="mb-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-5">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Question Database Dashboard
            </p>
          </div>
          <div className="flex items-center space-x-2 mt-4 md:mt-0">
            <button
              onClick={() => setDisplayMode("grid")}
              className={`p-1.5 rounded-md ${displayMode === "grid" ? "bg-gray-200 dark:bg-gray-700" : "text-gray-500 dark:text-gray-400"}`}
              title="Grid view"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-md ${displayMode === "list" ? "bg-gray-200 dark:bg-gray-700" : "text-gray-500 dark:text-gray-400"}`}
              title="List view"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          ...
          <StatsCard
            icon={<FaCloudUploadAlt className="w-4 h-4" />}
            label="Your Uploads"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          ...
          <QuickAction
            ...
            label="Questions"
            to="/questionbank"
            color="bg-gray-600 dark:bg-gray-700 text-white"
          />
          <QuickAction
            icon={<FaPlus className="text-white" />}
            label="Add New"
            to="/questionbank"
            color="bg-gray-800 dark:bg-gray-900 text-white"
          />
          <QuickAction
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        {/* Feature cards */}
        <div className={`lg:flex-1 ${displayMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" : "space-y-3"}`}>
          <DashboardCard
            icon={<FaQuestionCircle className="h-5 w-5 text-gray-700 dark:text-gray-300" />}
            title="Question Bank"
            ...
            actions={[
              ...
              { label: "Add New", path: "/questionbank", icon: <FaPlus className="w-3 h-3" /> }
            ]}
          />
          
          <DashboardCard
            icon={<FaCloudUploadAlt className="h-5 w-5 text-gray-700 dark:text-gray-300" />}
          ...
          <DashboardCard
            icon={<FaListAlt className="h-5 w-5 text-gray-700 dark:text-gray-300" />}
            title="My Questions"
          ...
          <DashboardCard
            icon={<FaLayerGroup className="h-5 w-5 text-gray-700 dark:text-gray-300" />}
          ...
          {user?.role === 'admin' && (
            <DashboardCard
              icon={<FaUsers className="h-5 w-5 text-gray-700 dark:text-gray-300" />}
          ...
          <DashboardCard
            icon={<FaChartBar className="h-5 w-5 text-gray-700 dark:text-gray-300" />}
            title="Examples & Templates"
            description="View example questions and templates to help standardize your content creation."
            color="bg-gray-100 dark:bg-gray-700/30 text-gray-700 dark:text-gray-300"
            path="/examples"
            actions={[
              { label: "View Examples", path: "/examples", icon: <FaChartBar className="w-3 h-3" /> }
            ]}
          />
        </div>
        ...
        <div className="lg:w-72 xl:w-80 shrink-0">
          <div className="sticky top-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-4">
            <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-3">Recent Activity</h3>
            ...
            {!isActivityLoading && activityToDisplay.length > 0 && activityToDisplay[0].type !== 'system' && (
              <div className="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                <button className="text-xs text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 font-medium">
                  View all activity
                </button>
              </div>
            )}
