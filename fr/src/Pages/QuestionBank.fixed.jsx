...
const QuestionBank = () => {
    ...
    useEffect(() => {
        if (editData?.class_id) {
            setFilteredEditSubjects(subjects.filter(s => s.class_id === editData.class_id));
        } else {
    ...
    useEffect(() => {
        if (editData?.subject_id) {
            setFilteredEditChapters(chapters.filter(c => c.subject_id === editData.subject_id));
        } else {
                ...
                <div className="pb-6">
                    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-md backdrop-blur-sm bg-opacity-95 dark:bg-opacity-95 transition-all duration-200 hover:shadow-lg">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
                            {["classname", "subject", "chapter", "topic", "difficulty_level", "reference"].map(field => (
                                <div key={field}>
                                    <select
                                        name={field}
                                        value={filters[field]}
                                    ...
                                    >
                                        <option value="">{`All ${field.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}`}</option>
                                        {uniqueValues(field).map((val, idx) => (
                                            <option key={`${field}-${val}-${idx}`} value={val}>{val}</option>
                                        ))}
                                    </select>
                                </div>
                            ))}
